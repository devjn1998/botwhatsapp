...
if ($method === 'POST') {   // Cria ou atualiza
    ...
    if ($id) { // ATUALIZAR (UPDATE)
        ...
        if (empty($data)) { // Se o quando eu enviar uma requisição POST e o corpo da requisição estiver vazio ele gera um erro
            ...
            echo json_encode(['error' => 'E necessario fornecer dados para atualizar a tarefa.']);
            exit();
        }
        ...
        if (empty($data) || !isset($data['content'])) {
            http_response_code(400);
            echo json_encode(['error' => 'O conteudo da tarefa e obrigatorio para criacao.']);
            exit();
        }
